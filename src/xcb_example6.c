...
int x_create_window(int x, int y, int w, int h) {
  ...
  cookie = xcb_create_window(
      connection, XCB_COPY_FROM_PARENT, window, screen->root, x, y, w, h, 0,
...
void x_fill_rectangle(int window, int graphics_context, int x, int y, int w,
                      int h) {
...
void x_draw_line(int window, int graphics_context, int x1, int y1, int x2,
                 int y2) {
  xcb_point_t p[2];
  ...
  xcb_poly_line(connection, XCB_COORD_MODE_ORIGIN, window, graphics_context, 2,
                p);
}

void x_paint_pixmap(int window, int graphics_context, int x, int y,
                    int bitmap_id) {
  int w, h;
  uint8_t *bitmap;
  xcb_pixmap_t pixmap;
  ...
  pixmap = xcb_create_pixmap_from_bitmap_data(
      connection, window, bitmap, w, h, screen->root_depth, screen->black_pixel,
      screen->white_pixel, NULL);

  xcb_copy_area(connection, pixmap, window, graphics_context, 0, 0, x, y, w, h);
}
